...
# -*- coding: utf-8 -*-
"""
Master Pipeline Orchestrator for Costco Category 8 (Food & Dining) Crawl
...
WORKSPACE_DIR = os.path.join(PROJECT_ROOT, ".agent_workspace", "c8_food_crawl")
STATE_FILE = os.path.join(WORKSPACE_DIR, "state.json")
REPORT_FILE = os.path.join(WORKSPACE_DIR, "report.md")

STEPS = [
    {
        "id": "step_01_build_catalog_tree",
        "script": "step_01_build_catalog_tree.py",
        "output": os.path.join(WORKSPACE_DIR, "catalog_manifest.json"),
        "args": lambda s, p: [
            "--output", s["output"]
        ]
    },
    {
        "id": "step_02_crawl_c8_products",
        "script": "step_02_crawl_c8_products.py",
        "input": os.path.join(WORKSPACE_DIR, "catalog_manifest.json"),
        "output": os.path.join(WORKSPACE_DIR, "c8_all_products_raw.json"),
        "args": lambda s, p: [
            "--input", s["input"],
            "--output", s["output"]
        ]
    ...
    {
        "id": "step_03_enrich_and_process",
        "script": "step_03_enrich_and_process.py",
        "input": os.path.join(WORKSPACE_DIR, "c8_all_products_raw.json"),
        "output": os.path.join(WORKSPACE_DIR, "c8_processed_products.json"),
        "args": lambda s, p: [
            "--input", s["input"],
            "--output", s["output"],
    ...
    {
        "id": "step_04_sync_db_and_frontend",
        "script": "step_04_sync_db_and_frontend.py",
        "input": os.path.join(WORKSPACE_DIR, "c8_processed_products.json"),
...
def generate_report(state, overall_status, durations):
    ...
    report_content = f"""# Execution Report — Costco C8 Food Crawl Pipeline
...
## Next Actions
{"All steps completed successfully. The frontend and SQLite database are fully synchronized." if not errors else "Inspect error logs, patch step scripts, and resume with --from-step."}
"""
...
def main():
    parser = argparse.ArgumentParser(description="Costco C8 Pipeline Orchestrator")
    ...
    print("================================================================")
    print("      Costco Taiwan Food & Dining (c/8) Pipeline Runner         ")
    print("================================================================")
    ...
    for s in target_steps:
        ...
        if res.returncode == 0 and os.path.exists(output_file):
            ...
            state[sid]["error"] = {
                ...
                "message": f"Script {s['script']} exited with non-zero code {res.returncode}",
                "traceback": None
            }
            save_state(state)
    ...
    if halted_step:
        ...
        overall_status = f"All {len(target_steps)} steps succeeded."
